...
const InputUpdatePassword = React.forwardRef((props, ref) => {
    	...
    	<React.Fragment>
		    {
			    (type === TYPE_PASSWORD || type === TYPE_CONFIRM_PASSWORD) ? (
				    <div className={ClassNames(styles['form-update-password-wrap'], className)}>
					    <div className={styles['form-update-password-input']}>
						    <input
							    ref={refInput}
							    ...
							    className={ClassNames(styles['form-update-password-input-pass'], showError() && styles['form-update-password-input-error'])}
						    />
						    <img alt='icon Eye' src={isView ? EyeView : EyeHide} className={styles.iconEye} onClick={onClickIconEye} />
					    </div>
					    {
						    showError() && <span className={styles['form-update-password-text-error']}>{showError()}</span>
					    }
				    </div>
			    ) : (
				    <div className={ClassNames(styles['form-update-password-input-text-wrap'], className)}>
					    <input
						    type={type}
						    ref={refInput}
						    onBlur={handleBlur}
						    onFocus={handleFocus}
						    placeholder={placeholder}
						    className={ClassNames(styles['form-update-password-input'], showError() && styles['form-update-password-input-error'])}
					    />
					    {
						    showError() && <span className={styles['form-update-password-text-error']}>{showError()}</span>
					    }
...
InputUpdatePassword.defaultProps = {
	type: 'text'
};
